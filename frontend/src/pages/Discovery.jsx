  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        // Fetch projects - assuming the backend returns all public projects by default when hitting /api/projects
        // We'll filter on the frontend for simplicity, or ideally backend should have a specific public search endpoint
        const { data: projData } = await axios.get('http://localhost:5000/api/projects', config);
        ...
        // Fetch users
        const { data: userData } = await axios.get(`http://localhost:5000/api/users?search=${query}`, config);
        setUsers(userData);
      } catch (error) {
                ...
                {users.length === 0 ? <p className={styles.empty}>No users found.</p> : users.map(u => (
                  <div key={u._id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div className={styles.avatar}>
                      {u.profilePicture ? <img src={u.profilePicture} alt="P" /> : <FiUsers size={24} />}
                    </div>
                    <div>
